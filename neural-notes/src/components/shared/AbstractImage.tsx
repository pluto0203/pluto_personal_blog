...
export function AbstractImage({ seed, className = "" }: AbstractImageProps) {
  ...
  return (
    <div
      className={`w-full h-48 bg-[#0a0a0a] relative overflow-hidden border-b border-white/[0.05] ${className}`}
    ...
    >
      <svg className="absolute inset-0 w-full h-full" xmlns="http://www.w3.org/2000/svg">
        ...
        {idx % 3 === 0 && (
          <path
            d={`M0,${60 + seed * 5} Q${80 + seed * 10},${30 + seed * 3} ${160 + seed * 5},${70 + seed * 4} T${320},${60 + seed * 3}`}
            fill="none"
        ...
        {idx % 2 === 1 && (
          <>
            ...
            <line
              x1={`${20 + seed * 7}%`} y1={`${40 + seed * 5}%`}
              x2={`${60 + seed * 5}%`} y2={`${30 + seed * 7}%`}
              stroke={strokeColors[idx]} strokeWidth="0.8" opacity="0.25" strokeDasharray="3 5"
            />
          </>
        )}
      </svg>
